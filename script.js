...
function gerarCSV(labels, mediaData, mediaMaisDesvioData, mediaMenosDesvioData) {
    let csv = "Labels,Media,Media+SD,Media-SD\n";
    for (let i = 0; i < labels.length; i++) {
        csv += `${labels[i]},${mediaData[i]},${mediaMaisDesvioData[i]},${mediaMenosDesvioData[i]}\n`;
    }
    return csv;
}
...
document.getElementById("stop").addEventListener("click", function() {
    ...
    baixarCSV(gerarCSV(labels, mediaData, mediaMaisDesvioData, mediaMenosDesvioData), "simulation_output.csv");
...
document.getElementById("startMigration").addEventListener("click", function() {
    ...
    // Adicione aqui a lógica para iniciar a migração
});
...
document.getElementById("stopMigration").addEventListener("click", function() {
    ...
    redesenharCirculos(true, false);
    // Adicione aqui a lógica para parar a migração
});
...
document.getElementById("startAttack").addEventListener("click", function() {
    ...
    // Adicione aqui a lógica para iniciar o ataque
});
...
document.getElementById("stopAttack").addEventListener("click", function() {
    ...
    redesenharCirculos(false, true);
    // Adicione aqui a lógica para parar o ataque
});
...
function printState(state) {
    ...
    // Desenha apenas as últimas linhas que cabem no canvas
...
async function loadData() {
    try {
        dadosNMNA = await carregarDadosArquivo('NMNA.csv');
        dadosYMNA = await carregarDadosArquivo('YMNA.csv');
        dadosNMYA = await carregarDadosArquivo('NMYA.csv');
        dadosYMYA = await carregarDadosArquivo('YMYA.csv');
...
async function carregarDadosArquivo(nomeArquivo) {
    ...
    return linhas.map(linha => {
        const [data, media, desvioPadrao] = linha.split(',').map(Number);
        return { data, media, desvioPadrao };
    });
}
...
desenharCirculo3(false);
desenharCirculo4(false);
atualizarGrafico(dadosNMNA); // Inicializa o gráfico
//state = "NMNA"
//redesenharCirculos();
